 ...
 */
async function verifyPlaidWebhook(body: string, signature: string | null): Promise<boolean> {
  const webhookSecret = Deno.env.get("PLAID_WEBHOOK_SECRET");

  if (!webhookSecret) {
    console.error("PLAID_WEBHOOK_SECRET environment variable is not set");
    return false;
  }
...
Deno.serve(async (req) => {
  try {
    ...
    // Log webhook event (omit raw payload to avoid storing sensitive data)
    await base44.asServiceRole.entities.PlaidWebhookLog.create({
      ...
      payload: JSON.stringify({
        ...
        item_id: webhook.item_id,
        transfer_id: webhook.transfer_id,
        transfer_status: webhook.transfer_status
      }),
      processed: true
    });
