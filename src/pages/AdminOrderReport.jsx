...
export default function AdminOrderReport() {
  ...
  const handleDownload = async () => {
    ...
    setMessage('');
    try {
      const response = await base44.functions.invoke('generateOrderReportPDF', {}, { responseType: 'arraybuffer' });
      const blob = new Blob([response.data], { type: 'application/pdf' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `RedHelixResearch_OrderReport_${new Date().toISOString().slice(0, 10)}.pdf`;
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-8">
      ...
      >
        ...
        <div className="space-y-2">
          ...
          <h1 className="text-2xl font-black text-slate-900">Order Report PDF</h1>
          <p className="text-slate-500 text-sm leading-relaxed">
            Generates a comprehensive PDF containing every order detail, customer info, shipping, payment, items, affiliate data, and full business statistics — ready for migration or archiving.
          </p>
        </div>
        ...
        <div className="bg-slate-50 rounded-xl p-4 text-left space-y-2">
          ...
          {[
            'Business summary (revenue, discounts, AOV)',
            'Orders breakdown by status & payment method',
            'Top 10 products by revenue',
            'Every order: customer, address, items, tracking',
            'All financial fields: subtotal, shipping, discounts',
            'Promo/affiliate codes, admin notes, record IDs',
          ].map((item, i) => (
        ...
        >
          {loading ? (
            ...
            <span className="flex items-center gap-2">
              ...
              Download Full Order Report PDF
            </span>
          )}
        </Button>
