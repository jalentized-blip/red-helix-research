...
export default function CryptoCheckout() {
  ...
  const processSuccessfulPayment = async (txHash, method = 'cryptocurrency') => {
    ...
    try {
      ...
      // Mark promo code as used — increments DB used_count and auto-deactivates if max_uses reached
      if (promoCode) {
        try {
          await base44.functions.invoke('validateOrder', { action: 'mark_promo_used', code: promoCode, orderNumber });
        } catch (e) { console.warn('mark_promo_used failed (non-blocking):', e); }
      }
                      ...
                      <Button
                        ...
                        saveCheckoutSnapshot({ ...buildSnapshotPayload('zelle'), zelleAccountName, zelleConfirmationNumber }).catch(() => {});
                          ...
                          try {
                            ...
                            // Decrement stock immediately on Zelle order submission
                            try { await decrementStock(cartItems); } catch (stockErr) { console.warn('Zelle stock decrement failed (non-blocking):', stockErr); }
                            // Mark promo as used
                            if (promoCode) {
                              try { await base44.functions.invoke('validateOrder', { action: 'mark_promo_used', code: promoCode, orderNumber: orderNumberRef.current }); } catch {}
                            }
                            clearCart();
                            // Admin notification
                            try {
                             ...
                             try {
                              ...
                              try {
                                ...
                                // Customer confirmation email (Square — sent immediately on order placement)
                                 ...
                                 // Mark promo as used at order placement (before payment confirmed)
                                    // This prevents the code being reused during the payment window
                                    if (promoCode) {
                                      try { await base44.functions.invoke('validateOrder', { action: 'mark_promo_used', code: promoCode, orderNumber: orderNumberRef.current }); } catch {}
                                    }
