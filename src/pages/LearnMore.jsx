...
export default function LearnMore() {
  ...
  const categoryColors = {
    weight_loss: 'from-orange-600 to-[#8B2635]',
    ...
    sexual_health: 'from-rose-600 to-[#8B2635]',
    ...
    <div className="min-h-screen bg-white pt-32 pb-20">
      ...
      <Dialog open={!disclaimerAccepted} onOpenChange={() => {}}>
        <DialogContent className="bg-white border border-slate-200 max-w-md rounded-[32px] p-8">
          <DialogHeader>
            <DialogTitle className="text-black text-2xl font-black uppercase tracking-tight">Research <span className="text-[#8B2635]">Protocol</span></DialogTitle>
          </DialogHeader>
          ...
          <div className="flex gap-3 mt-8">
            <Button
              ...
              className="flex-1 bg-[#8B2635] hover:bg-[#6B1827] text-white font-black uppercase tracking-widest text-xs h-14 rounded-2xl shadow-lg shadow-[#8B2635]/20"
            >
              I Understand
            </Button>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="mb-16">
          <Link to={createPageUrl('Home')}>
            <Button variant="outline" className="border-slate-200 text-slate-400 hover:text-[#8B2635] hover:border-[#8B2635] mb-8 rounded-xl font-black uppercase tracking-widest text-[10px]">
              ← Back to Shop
            </Button>
          </Link>
          <div className="mb-12">
            <div className="flex items-center gap-4 mb-4">
              <div className="p-3 bg-[#8B2635]/10 rounded-2xl">
                <BookOpen className="w-8 h-8 text-[#8B2635]" />
              </div>
              <h1 className="text-6xl font-black text-black uppercase tracking-tighter">Learn <span className="text-[#8B2635]">More</span></h1>
            </div>
          ...
          {filteredProducts.map((product, idx) => (
            ...
            >
              <Link to={`${createPageUrl('PeptideLearn')}?id=${product.id}&name=${encodeURIComponent(product.name)}`}>
                <div className="bg-white border border-slate-100 rounded-[32px] p-8 h-full cursor-pointer relative overflow-hidden shadow-sm hover:shadow-xl hover:border-[#8B2635]/20 transition-all duration-500">
                  {/* Category Accent */}
                  ...
                  <div className="relative z-10 flex flex-col h-full">
                    <div className="mb-6">
                      <span className="inline-block px-3 py-1 bg-[#8B2635]/10 rounded-full text-[10px] font-black text-[#8B2635] uppercase tracking-widest mb-4">
                        {categoryLabels[product.category]}
                      </span>
                      <h3 className="text-3xl font-black text-black mb-3 uppercase tracking-tight group-hover:text-[#8B2635] transition-colors">
                        {product.name}
                      </h3>
                    ...
                    <div className="mt-auto pt-6 border-t border-slate-50 flex flex-col gap-4">
                      ...
                      <div className="flex gap-3">
                        <button
                          ...
                          className="flex-1 px-4 py-3 bg-[#8B2635] hover:bg-[#6B1827] text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all shadow-lg shadow-[#8B2635]/10"
                        >
                          Initialize Order
                        </button>
        ...
        {filteredProducts.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-slate-500 text-lg font-medium">No peptides found in this category.</p>
          </div>
        )}
      </div>
    </div>
