    ...
    <div className="min-h-screen bg-stone-950 pt-32 pb-20">
      ...
      <div className="max-w-5xl mx-auto px-4">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-12">
          <h1 className="text-5xl md:text-6xl font-black text-amber-50 mb-4">Conversion Tracking Setup</h1>
          <p className="text-xl text-stone-300">Measure every step of the customer journey</p>
        </motion.div>
        ...
        <motion.div
          ...
          className="bg-stone-900/60 border border-stone-700 rounded-lg p-8 mb-12"
        >
          <h2 className="text-2xl font-bold text-amber-50 mb-8">Expected Conversion Funnel</h2>
          <div className="space-y-4">
            {conversionFunnel.map((item, idx) => (
              <div key={idx} className="flex items-center gap-4">
                <div className="flex-1">
                  <p className="font-semibold text-amber-50">{item.stage}</p>
                  <p className="text-sm text-stone-400">{item.metric}</p>
                </div>
                <div className="relative h-8 bg-stone-800/50 rounded flex-1 flex items-center justify-end pr-3">
                  <motion.div
                    initial={{ width: 0 }}
                    whileInView={{ width: item.value }}
                    transition={{ delay: idx * 0.1, duration: 0.5 }}
                    className="absolute left-0 h-full bg-red-700/30 rounded"
                  />
                  <span className="text-amber-50 font-bold relative z-10">{item.value}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        >
          <h2 className="text-2xl font-bold text-amber-50 mb-6">Key Events to Implement</h2>
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-stone-700">
                <th className="text-left py-3 px-4 text-amber-50 font-bold">Event</th>
                <th className="text-left py-3 px-4 text-amber-50 font-bold">Implementation Code</th>
                <th className="text-left py-3 px-4 text-amber-50 font-bold">Importance</th>
              </tr>
            </thead>
            <tbody className="text-stone-300">
              {trackingEvents.map((event, idx) => (
                <tr key={idx} className="border-b border-stone-700/50 hover:bg-stone-800/30 transition">
                  <td className="py-4 px-4 font-semibold text-amber-50 whitespace-nowrap">{event.event}</td>
                  <td className="py-4 px-4">
                    <code className="text-xs bg-stone-950 rounded px-2 py-1 font-mono overflow-x-auto block">
                      {event.code}
                    </code>
                  </td>
                  <td className="py-4 px-4">
          ...
          {googleAnalyticsSetup.map((section, idx) => (
            ...
            >
              <div className="flex items-center gap-4 mb-6">
                <div className="flex items-center justify-center h-12 w-12 rounded-full bg-red-700 text-amber-50 font-bold text-lg">
                  {section.step}
                </div>
