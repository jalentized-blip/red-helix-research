...
const StorySection = ({ icon: Icon, title, description, highlight, highlightSecondary }) => (
  <motion.div
    ...
    className="bg-white border border-slate-200 rounded-[40px] p-8 md:p-12 shadow-xl shadow-slate-100 hover:border-[#8B2635]/30 transition-all group relative overflow-hidden"
  >
    ...
    <div className="flex flex-col md:flex-row items-start gap-8 relative z-10">
      <div className="w-16 h-16 bg-[#8B2635] rounded-2xl flex items-center justify-center flex-shrink-0 shadow-lg shadow-[#8B2635]/20 group-hover:scale-110 transition-transform">
        <Icon className="w-8 h-8 text-white" />
      </div>
      <div>
        <h3 className="text-3xl font-black text-black mb-4 uppercase tracking-tighter leading-none group-hover:text-[#8B2635] transition-colors">
          {title}
        </h3>
        ...
        {highlight && (
          <div className="inline-flex flex-col gap-1 px-6 py-3 bg-slate-50 border border-slate-100 rounded-2xl">
            ...
            <span className="text-lg font-black text-[#8B2635] uppercase tracking-tight">{highlight}</span>
          </div>
        )}
      </div>
    </div>
...
export default function About() {
  ...
  return (
    <div className="min-h-screen bg-white pt-32 pb-20 relative overflow-hidden">
      <SEO
        title="About Red Helix Research — USA Peptide Supplier with COA Verification"
        description="Red Helix Research is a USA-based supplier of HPLC-verified research peptides. Third-party COA on every batch. Transparent pricing, fast shipping, and dedicated researcher support."
        keywords="about red helix research, research peptide supplier USA, laboratory peptides, peptide vendor, COA verified, HPLC tested, research peptide company, trusted peptide supplier"
        canonical="https://redhelixresearch.com/About"
        schema={schemas}
      />
      ...
      <div className="max-w-6xl mx-auto px-4 mb-20 relative z-10 text-center">
        ...
        >
          ...
          <h1 className="text-6xl md:text-8xl font-black text-black mb-8 uppercase tracking-tighter leading-none">
            Our <span className="text-[#8B2635]">Mission</span>
          </h1>
          ...
          <div className="inline-flex items-center gap-4 px-8 py-4 bg-[#fef2f2] border border-[#fee2e2] rounded-3xl shadow-lg shadow-[#fee2e2]/50">
            <ShieldCheck className="w-6 h-6 text-[#8B2635]" />
            <span className="text-sm font-black text-[#8B2635] uppercase tracking-widest">
        ...
        {[
          ...
          >
            <div className="w-12 h-12 bg-white rounded-xl shadow-sm flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform">
              <stat.icon className="w-6 h-6 text-[#8B2635]" />
            </div>
        ...
        <div className="bg-white border border-slate-200 rounded-[40px] p-10 md:p-20 text-center relative overflow-hidden shadow-xl shadow-slate-200/50">
          ...
          >
            <h2 className="text-4xl md:text-6xl font-black text-black mb-8 uppercase tracking-tighter leading-none">
              The Red Helix <span className="text-[#8B2635]">Standard</span>
            </h2>
            ...
            <div className="grid md:grid-cols-2 gap-4 max-w-2xl mx-auto mb-12">
              {[
                ...
                <div key={idx} className="flex items-center gap-3 text-left p-4 bg-slate-50 border border-slate-200 rounded-2xl hover:border-[#8B2635]/30 transition-colors">
                  <div className="w-6 h-6 rounded-full bg-[#8B2635] flex items-center justify-center flex-shrink-0">
            ...
            <Link to={createPageUrl('Home')}>
              <Button size="lg" className="bg-[#8B2635] hover:bg-[#6B1827] text-white px-12 py-8 rounded-full font-black uppercase tracking-widest text-sm shadow-xl shadow-[#8B2635]/20">
