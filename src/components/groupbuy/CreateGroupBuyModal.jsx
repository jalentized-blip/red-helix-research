            ...
            <div className="grid grid-cols-1 gap-2">
              {TEST_OPTIONS.map(t => (
                <button key={t.id} onClick={() => toggleTest(t.id)}
                  className={`flex items-center gap-4 p-3 rounded-xl border text-left transition-all ${form.test_types.includes(t.id) ? 'border-[#8B2635] bg-[#8B2635]/5' : 'border-slate-200 hover:border-slate-300'}`}>
                  <div className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 ${form.test_types.includes(t.id) ? 'bg-[#8B2635] border-[#8B2635]' : 'border-slate-300'}`}>
                    {form.test_types.includes(t.id) && <CheckCircle2 className="w-3.5 h-3.5 text-white" />}
                  </div>
                  <div>
                    <div className="font-bold text-sm text-black">{t.label}</div>
