...
export default function ProtocolEngine() {
  ...
  return (
    <section className="py-24 px-4 bg-neutral-950 relative overflow-hidden text-white">
      {/* Grid Background */}
      <div className="absolute inset-0 bg-[linear-gradient(to_right,#262626_1px,transparent_1px),linear-gradient(to_bottom,#262626_1px,transparent_1px)] bg-[size:40px_40px] opacity-20" />
      
      {/* Animated Gradient Overlay */}
      <div className="absolute inset-0 bg-gradient-to-b from-neutral-950/0 via-neutral-950/50 to-neutral-950 pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 h-full min-h-[600px]">
          ...
          <div className="lg:col-span-4 flex flex-col gap-6">
            <div className="bg-neutral-900/50 backdrop-blur-sm border border-neutral-800 rounded-2xl p-6 shadow-xl">
              <div className="flex items-center gap-3 mb-6 border-b border-neutral-800 pb-4">
                ...
                <h3 className="font-mono text-sm text-neutral-400 tracking-widest">PROTOCOL_ENGINE_V2.1</h3>
              </div>
              
              <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-neutral-700 scrollbar-track-neutral-900/50">
                <p className="text-neutral-400 text-sm mb-4 font-mono sticky top-0 bg-neutral-900/95 backdrop-blur-sm z-10 py-2 border-b border-neutral-800">SELECT RESEARCH OBJECTIVE:</p>
                {Object.entries(protocols).map(([key, protocol]) => (
                  ...
                  >
                    <div className={`p-2 rounded-lg ${activeProtocol?.id === key ? 'bg-white/20' : 'bg-neutral-700 group-hover:bg-neutral-600'}`}>
                      <protocol.icon className="w-5 h-5" />
                    </div>
                    <div className="flex-1">
                      <div className="font-bold">{protocol.name}</div>
                      <div className={`text-xs ${activeProtocol?.id === key ? 'text-red-100' : 'text-neutral-500'}`}>
              ...
              {/* System Status */}
              <div className="mt-8 p-4 bg-neutral-950 rounded-lg border border-neutral-800 font-mono text-xs text-neutral-500 space-y-2">
          ...
          <div className="lg:col-span-8 bg-neutral-900/30 backdrop-blur-sm border border-neutral-800 rounded-2xl p-8 relative overflow-hidden flex flex-col">
            {/* Header Details */}
            <div className="flex justify-between items-start mb-8">
              <div>
                <h2 className="text-2xl font-black tracking-tight mb-2 text-red-600">ADVANCED PROTOCOL SIMULATION</h2>
                <p className="text-neutral-400 font-mono text-sm">Visualize synergy and mechanism of action.</p>
              </div>
              <div className="flex gap-2">
                <div className="px-3 py-1 bg-neutral-800 rounded text-xs font-mono text-neutral-400 border border-neutral-700">
                  MODE: INTERACTIVE
                </div>
              </div>
            </div>
                ...
                {!activeProtocol && !isScanning && (
                  ...
                  >
                    <div className="relative w-32 h-32 mx-auto mb-6">
                      <div className="absolute inset-0 border-2 border-neutral-800 rounded-full animate-ping opacity-20" />
                      <div className="absolute inset-0 border border-neutral-800 rounded-full animate-[spin_10s_linear_infinite]" />
                      <div className="absolute inset-4 border border-neutral-800 rounded-full animate-[spin_8s_linear_infinite_reverse]" />
                      <div className="absolute inset-0 flex items-center justify-center">
                        <ScanLine className="w-12 h-12 text-neutral-700" />
                      </div>
                    </div>
                ...
                {activeProtocol && (
                  ...
                  >
                    ...
                    <div className="relative group">
                      ...
                      <div className="relative bg-neutral-900 border border-neutral-700 p-6 rounded-2xl h-full flex flex-col items-center justify-center text-center">
                        ...
                        <p className="text-neutral-400 text-sm mb-4">{activeProtocol.description}</p>
                    ...
                    <div className="flex flex-col gap-4">
                      <h4 className="font-mono text-xs text-neutral-500 uppercase mb-2">Synergistic Compounds Detected:</h4>
                      {activeProtocol.synergistic.map((compound, idx) => (
                        <motion.div
                          ...
                          className="bg-neutral-800/50 border border-neutral-700 p-4 rounded-xl flex items-center justify-between group hover:border-red-500/50 transition-colors"
                        >
                          <div className="flex items-center gap-3">
                            <div className="w-2 h-2 rounded-full bg-red-500 shrink-0" />
                            <div>
                              <div className="font-bold text-neutral-200">{compound.name}</div>
                              <div className="text-xs text-neutral-500 font-mono group-hover:text-red-400 transition-colors">
                                {compound.effect}
                              </div>
                            </div>
                          </div>
                          <div className="text-[10px] text-neutral-600 font-mono uppercase tracking-wider hidden sm:block">
                            + SYNERGY
                          </div>
                        </motion.div>
                      ))}
                      ...
                      {/* Live Citation Feed */}
                      <div className="mt-auto bg-neutral-950 p-4 rounded-lg border border-neutral-800">
                        ...
                        <div className="space-y-2">
                          {activeProtocol.citations.map((cite, i) => (
                            <div key={i} className="text-[10px] text-neutral-400 border-l-2 border-neutral-800 pl-2 hover:border-red-500 transition-colors">
                              {cite}
                            </div>
                          ))}
                        </div>
            ...
            <div className="absolute bottom-0 left-0 p-4 opacity-50">
               <Binary className="w-4 h-4 text-neutral-700" />
            </div>
