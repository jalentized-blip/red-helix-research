...
const Hero = React.memo(() => {
  ...
  return (
    <section className="relative min-h-[100vh] flex items-center overflow-hidden pt-12">
      ...
      <motion.div 
        className="absolute top-1/4 right-1/4 w-64 h-64 bg-red-600/5 rounded-full blur-[100px]"
        animate={{ 
          scale: [1, 1.1, 1],
          opacity: [0.3, 0.4, 0.3],
          x: [0, 30, 0],
      ...
      />
      <motion.div 
        className="absolute bottom-1/4 left-1/4 w-80 h-80 bg-blue-400/5 rounded-full blur-[120px]"
        animate={{ 
          scale: [1.1, 1, 1.1],
          opacity: [0.2, 0.3, 0.2],
          x: [0, -30, 0],
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-12 py-32 flex flex-col lg:flex-row items-center gap-16">
        <motion.div 
        ...
        >
          ...
          <motion.div 
            ...
            className="inline-flex items-center gap-2 px-4 py-2 bg-white/80 backdrop-blur-md border border-slate-200 rounded-full mb-8 shadow-sm"
          >
            <ShieldCheck className="w-4 h-4 text-red-600" />
            <span className="text-xs font-bold tracking-widest text-slate-600 uppercase">ISO 9001:2015 Certified Sourcing</span>
          </motion.div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-6 mb-12">
            {[
              ...
              <div key={i} className="flex flex-col items-center lg:items-start gap-1">
                ...
                <span className="text-[10px] text-slate-500 uppercase tracking-tighter font-bold">{stat.sub}</span>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
            <Button 
              ...
              className="group bg-red-600 hover:bg-red-700 text-white px-10 py-8 text-lg font-black rounded-2xl shadow-[0_10px_30px_-5px_rgba(220,38,38,0.3)] hover:shadow-[0_15px_40px_-5px_rgba(220,38,38,0.4)] transition-all duration-300 transform hover:-translate-y-1"
            >
              <ShoppingBag className="w-6 h-6 mr-3 group-hover:rotate-12 transition-transform" />
              EXPLORE CATALOG
            </Button>
            <Button 
              variant="outline"
              onClick={() => scrollTo('certificates')}
              className="bg-white border-2 border-slate-200 text-slate-900 hover:bg-slate-50 hover:border-red-600 px-10 py-8 text-lg font-black rounded-2xl transition-all duration-300 shadow-sm"
            >
              VIEW LAB REPORTS
            </Button>
          </div>
